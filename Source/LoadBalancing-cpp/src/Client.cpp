...
#	pragma warning(pop)
		...
		bool Client::opJoinRandomOrCreateRoom(const JString& gameID, const RoomOptions& options, const Hashtable& customRoomProperties, nByte maxPlayers, nByte matchmakingMode, const JString& lobbyName, nByte lobbyType, const JString& sqlLobbyFilter, const JVector<JString>& expectedUsers)
		{
			...
			OperationRequestParameters op = mpPeer->opJoinRandomRoomImplementation(mRoomName=gameID, options, true, customRoomProperties, maxPlayers, matchmakingMode, lobbyName, lobbyType, sqlLobbyFilter, expectedUsers);
			if(getLocalPlayer().getName().length())
			{
				if((ValueObject<Hashtable>*)op.getValue(ParameterCode::PLAYER_PROPERTIES))
					((ValueObject<Hashtable>*)op.getValue(ParameterCode::PLAYER_PROPERTIES))->getDataAddress()->put(Properties::Player::PLAYERNAME, getLocalPlayer().getName());
