...
namespace ExitGames
{
	namespace LoadBalancing
	{
		...
		OperationRequestParameters Peer::enterRoomImplementation(const RoomOptions* pOptions, const Hashtable& customLocalPlayerProperties, const JVector<JString>& expectedUsers, bool usingFilters)
		{
			OperationRequestParameters op;
			...
			if(pOptions)
			{
				...
				op.put(usingFilters?ParameterCode::PROPERTIES:ParameterCode::ROOM_PROPERTIES, ValueObject<Hashtable>(roomProps)); // opJoinRandomRoom() already uses ParameterCode::ROOM_PROPERTIES for the room properties filter, so for backwards compatibilty reasons, opJoinRandomOrCreateRoom(), which is implemented by the same operation (just with 'createIfNotExists'==true) uses a different key (ParameterCode::PROPERTIES) for the room properties that are used for room creation
